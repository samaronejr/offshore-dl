...
\begin{enumerate}
    \item A \textbf{systematic benchmark} of nine architectures---six trained (LSTM, DeepONet, Random Forest, PatchTST, TCN, FKM-AD) and three foundation models (Chronos, TimesFM, TiRex)---across two real-world offshore datasets covering fault classification and production forecasting.
...
Table~\ref{tab:3w_main} presents the primary results on the held-out test set ($n{=}41{,}515$). All seven model configurations are evaluated on identical test samples. DeepONet and LSTM achieve near-identical accuracy ($\sim$96.8\%), while PatchTST reaches 96.5\% after Optuna HPO (30 trials, improved from 95.7\% with manual tuning). Notably, Random Forest achieves 96.58\% accuracy (0.966, macro $F_1{=}0.964$, AUC-PR${=}0.985$) on the flattened 378-dimensional feature vector, demonstrating that the statistical feature extraction pipeline captures discriminative information accessible even to non-sequential, tabular learners. FKM-AD on features achieves 96.6\% accuracy ($F_1$m${=}0.961$), competitive with the top models, while the raw-window variant drops to 92.0\%---a 4.6 percentage point gap that quantifies the value of the feature extraction pipeline for state-space models. Inner CV estimates are within 2\% of test performance for all trained models, indicating stable generalization.
...
FKM-AD (feat.)  & .966 & .961 & .966 & .927 & .954$\pm$.019 \\
...
\bottomrule
\multicolumn{6}{@{}l}{\scriptsize $^\dagger$After Optuna HPO (30 trials). DeepONet: 33 trials. FKM-AD uses pre-HPO baseline parameters.}
\end{tabular}
\end{table}
...
Fig.~\ref{fig:recall} shows per-class recall on the held-out test set for all six trained models (plus TiRex). Classes~5 (flow instability), 6 (rapid productivity loss), and 9 (hydrate formation) achieve $>$99\% recall across most trained models. Random Forest achieves the highest recall on class~2 (incipient BSW increase, 93.2\%), the rarest class, outperforming all neural architectures on this challenging category. FKM-AD (features) achieves 100\% recall on class~6 and 99.8\% on class~9, closely tracking the top models, but shows reduced recall on class~2 (89.9\%) and class~4 (92.2\%). The raw-window variant follows a similar pattern with uniformly lower recall, confirming that the SSM benefits substantially from pre-extracted features. PatchTST shows reduced recall on class~8 (slow restriction, 87\%), while TiRex shows the largest drops on class~2 (67.8\%) and class~4 (73.5\%), indicating that frozen embeddings lose fine-grained discriminative information for rare event types.
...
\legend{LSTM, DeepONet, PatchTST, RF, FKM-AD, TiRex}
\end{axis}
\end{tikzpicture}
\caption{Per-class recall on the 3W held-out test set for all six trained models plus TiRex. Classes: 0=Normal, 1=Abrupt BSW$\uparrow$, 2=Incipient BSW$\uparrow$, 3=Spurious closure, 4=Severe slugging, 5=Flow instability, 6=Productivity loss, 7=Quick restriction, 8=Slow restriction, 9=Hydrate.}
\label{fig:recall}
\end{figure}
...
FKM-AD provides further evidence from the state-space model family. On the pre-extracted feature matrix, FKM-AD achieves 96.6\% accuracy (CV${=}95.4\%$) with only 0.26M parameters---competitive with architectures 2--9$\times$ larger---while the raw-window variant drops to 92.0\% (CV${=}93.8\%$). This 4.6~pp gap between feature and raw paths quantifies the benefit of statistical feature extraction for SSMs: even Mamba's selective state-space mechanism, designed for long-range sequence modeling, performs substantially better when temporal dynamics are pre-encoded as statistical descriptors. The dual-path result complements Random Forest's success---both demonstrate that the feature extraction pipeline, not the downstream architecture's inductive bias, is the primary driver of classification performance.
...
We apply the Friedman test with Nemenyi post-hoc analysis to the inner CV fold-level metrics (5 folds for 3W, 3 folds for Ganymede). For 3W classification with seven model configurations (including both FKM-AD paths), the Friedman test is significant across all three metrics: accuracy ($\chi^2{=}23.3$, $p < 0.001$), macro $F_1$ ($\chi^2{=}25.5$, $p < 0.001$), and AUC-PR ($\chi^2{=}25.0$, $p < 0.001$). Average accuracy ranks place Random Forest first (1.0), PatchTST second (3.0), DeepONet third (3.6), FKM-AD~features fourth (3.8), LSTM fifth (4.0), FKM-AD~raw sixth (5.6), and TiRex seventh (7.0). With Nemenyi CD${}=4.03$ at $\alpha{=}0.05$, significant pairwise differences are found between Random Forest and FKM-AD~raw (rank diff$=4.6$) and between Random Forest and TiRex (rank diff$=6.0$), confirming that FKM-AD on features is statistically indistinguishable from the top models while the raw-window variant performs significantly worse than Random Forest.
...
\begin{enumerate}
    ...
    \item \textbf{SSMs benefit from feature extraction}: FKM-AD (Fourier-KAN Mamba) achieves 96.6\% accuracy on pre-extracted features but drops to 92.0\% on raw windows---a 4.6~pp gap demonstrating that even selective state-space models, designed for long-range sequence modeling, perform substantially better when temporal dynamics are pre-encoded as statistical descriptors.
