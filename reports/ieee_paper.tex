...
\begin{enumerate}
    \item A \textbf{systematic benchmark} of seven architectures---four trained (LSTM, DeepONet, MLP, PatchTST) and three foundation models (Chronos, TimesFM, TiRex)---across two real-world offshore datasets covering fault classification and production forecasting.
...
\end{axis}
\end{tikzpicture}
\caption{$R^2_\text{prod}$ vs.\ forecast horizon (Ganymede, held-out test, multi-well). Foundation models (top 3 curves) consistently outperform trained models. DeepONet omitted ($R^2 < -5$). Dashed line: $R^2{=}0$ (naive baseline).}
\label{fig:ganymede}
\end{figure}
...
This study has several limitations: (i) hyperparameter optimization was performed for the 3W classification task but not for Ganymede forecasting, where the large gap between FMs and trained models may partly reflect suboptimal trained-model tuning; and (ii) no interpretability analysis (e.g., attention visualization, SHAP values) has been conducted.
...
We benchmarked seven deep learning architectures on offshore fault classification and gas production forecasting using a rigorous nested evaluation protocol. Key findings:
...
Future work includes Ganymede hyperparameter optimization, model interpretability analysis, and evaluation of domain-adapted foundation models.
