...
We evaluate ConvTimeNet on the \textit{feature} path only ($14 \times 27$ statistical matrix), as its patch-based embedding is designed for compact representation inputs rather than raw 720-step sequences. Hyperparameters are optimized via 30-trial Optuna HPO on the feature path, consistent with the other HPO-tuned models.
...
ConvTimeNet \cite{convtimenet} is evaluated on the feature path only, achieving 96.6\% accuracy ($F_1$m${=}0.962$, AUC-PR${=}0.930$, CV${=}95.5\%\pm1.5\%$), ranking third among all configurations after Random Forest and FKM-AD~(feat.). Its pure-convolution design---deformable patch embedding with structural re-parameterization---matches the performance of attention- and SSM-based architectures on the pre-extracted feature matrix, demonstrating that the feature extraction pipeline enables diverse inductive biases to reach competitive accuracy.
...
ConvTimeNet$^\dagger$ & .966 & .962 & .966 & .930 & .955$\pm$.015 \\
...
\multicolumn{6}{@{}l}{\scriptsize $^\dagger$MambaSL, ConvTimeNet hyperparameters tuned via 30-trial Optuna HPO on feature path. ConvTimeNet: feature path only.}
\end{tabular}
\end{table}
...
ConvTimeNet \cite{convtimenet} introduces a third inductive bias family---pure convolution---into the feature-path analysis. Its deformable patch embedding with per-patch offset prediction and multi-scale depthwise convolution blocks with structural re-parameterization achieve 96.6\% accuracy (rank~3.8 of 10 configurations, third after Random Forest and FKM-AD~features), competitive with both attention-based and SSM-based architectures without using any self-attention or recurrent mechanisms. ConvTimeNet was evaluated exclusively on the feature path ($14 \times 27$), consistent with its patch-based design. Its result further supports the thesis that feature extraction is the primary performance driver: architectures spanning the full inductive bias spectrum---tabular (RF), SSM (FKM-AD, MambaSL), convolution (ConvTimeNet), and attention (PatchTST)---all converge to the same accuracy band (96.5--96.7\%) on the pre-extracted feature matrix.
...
We apply the Friedman test with Nemenyi post-hoc analysis to the inner CV fold-level metrics (5 folds for 3W, 3 folds for Ganymede). For 3W classification with ten model configurations (including both FKM-AD and MambaSL paths, plus ConvTimeNet), the Friedman test is significant across all three metrics: accuracy ($\chi^2{=}38.5$, $p < 0.001$), macro $F_1$ ($\chi^2{=}36.2$, $p < 0.001$), and AUC-PR ($\chi^2{=}36.1$, $p < 0.001$). Average accuracy ranks place Random Forest first (1.0), FKM-AD~features second (3.0), ConvTimeNet third (3.8), PatchTST fourth (4.0), DeepONet fifth (4.8), LSTM sixth (5.6), MambaSL~features seventh (5.8), FKM-AD~raw eighth (8.4), MambaSL~raw ninth (8.6), and TiRex tenth (10.0). With Nemenyi CD${}=6.06$ at $\alpha{=}0.05$, significant pairwise differences are found between: ConvTimeNet and TiRex (rank diff$=6.2$); FKM-AD~features and TiRex (rank diff$=7.0$); FKM-AD~raw and Random Forest (rank diff$=7.4$); MambaSL~raw and Random Forest (rank diff$=7.6$); and Random Forest and TiRex (rank diff$=9.0$). These results confirm that all feature-path models are statistically indistinguishable from each other, while both raw-window variants and TiRex form a separate lower tier.
...
\begin{enumerate}
    ...
    \item \textbf{ConvTimeNet confirms architecture-agnostic feature extraction}: ConvTimeNet \cite{convtimenet} (pure-convolution, deformable patches with structural re-parameterization) achieves 96.6\% accuracy on pre-extracted features (rank~3.8 of 10 configurations), demonstrating that even architectures without attention or SSM mechanisms match top performance when paired with the feature extraction pipeline. The convergence of tabular (RF), SSM (FKM-AD, MambaSL), convolution (ConvTimeNet), and attention (PatchTST) models to the 96.5--96.7\% accuracy band validates feature extraction as the decisive factor.
