...
\begin{enumerate}
    \item A \textbf{systematic benchmark} of ten architectures---five trained (LSTM, DeepONet, Random Forest, PatchTST, TCN) and three foundation models (Chronos, TimesFM, TiRex)---across two real-world offshore datasets covering fault classification and production forecasting.
...
Table~\ref{tab:3w_main} presents the primary results on the held-out test set ($n{=}41{,}515$). All five models are evaluated on identical test samples. DeepONet and LSTM achieve near-identical accuracy ($\sim$96.8\%), while PatchTST reaches 96.5\% after Optuna HPO (30 trials, improved from 95.7\% with manual tuning). Notably, Random Forest achieves 96.58\% accuracy on the flattened 378-dimensional feature vector, demonstrating that the statistical feature extraction pipeline captures discriminative information accessible even to non-sequential, tabular learners. Inner CV estimates are within 2\% of test performance for all trained models, indicating stable generalization.
...
\midrule
% TODO: S04 will fill TCN results after training
\multirow{2}{*}{TCN}
 & $R^2_\text{prod}$ & -- & -- & -- & -- \\
 & MAE              & -- & -- & -- & -- \\
\bottomrule
...
% TODO: TCN plot coordinates pending S04 training
\addplot[dashed, gray, domain=0:95] {0};
\legend{TimesFM, TiRex, Chronos, PatchTST, LSTM}
\end{axis}
\end{tikzpicture}
\caption{$R^2_\text{prod}$ vs.\ forecast horizon (Ganymede, held-out test, multi-well). Foundation models (top 3 curves) consistently outperform trained models. DeepONet omitted ($R^2 < -5$). TCN results pending (S04). Dashed line: $R^2{=}0$ (naive baseline).}
\label{fig:ganymede}
\end{figure}
...
This study has several limitations: (i) hyperparameter optimization was performed for the 3W classification task but not for the trained models on Ganymede forecasting, where the large gap between FMs and trained models may partly reflect suboptimal tuning; (ii) no interpretability analysis (e.g., attention visualization, SHAP values) has been conducted; and (iii) TCN training on Ganymede is pending---its results will be incorporated in the final version.
...
\begin{enumerate}
    ...
    \item \textbf{TCN for Ganymede forecasting}: As a sequential architecture with dilated causal convolutions, TCN is expected to bridge the gap between tabular-input trained models and foundation models; results pending.
\end{enumerate}

Future work includes TCN training and evaluation on Ganymede, hyperparameter optimization for the remaining trained models on forecasting, model interpretability analysis, and evaluation of domain-adapted foundation models.
