...
We apply the Friedman test with Nemenyi post-hoc analysis to the inner CV fold-level metrics (5 folds for 3W, 3 folds for Ganymede). For 3W classification with seven model configurations (including both FKM-AD paths), the Friedman test is significant across all three metrics: accuracy ($\chi^2{=}26.9$, $p < 0.001$), macro $F_1$ ($\chi^2{=}25.5$, $p < 0.001$), and AUC-PR ($\chi^2{=}25.2$, $p < 0.001$). Average accuracy ranks place Random Forest first (1.0), FKM-AD~features second (2.6), PatchTST third (3.0), DeepONet fourth (4.0), LSTM fifth (4.4), FKM-AD~raw sixth (6.0), and TiRex seventh (7.0). With Nemenyi CD${}=4.03$ at $\alpha{=}0.05$, significant pairwise differences are found between Random Forest and FKM-AD~raw (rank diff$=5.0$) and between Random Forest and TiRex (rank diff$=6.0$), confirming that FKM-AD on features is statistically indistinguishable from the top models while the raw-window variant performs significantly worse than Random Forest.
