...
\definecolor{xgboost}{HTML}{8B4513}
...
% ═══════════════════════════════════════════════════════════════════
\begin{abstract}
Digital monitoring of offshore oil and gas operations demands models capable of detecting equipment faults and forecasting production volumes under challenging conditions: high class imbalance, limited labeled data, multi-well heterogeneity, and long forecast horizons. We present a systematic benchmark of eight deep learning architectures---five trained models (Bidirectional LSTM, DeepONet, MLP, PatchTST, XGBoost) and three foundation models (Chronos, TimesFM, TiRex)---evaluated on two real-world offshore datasets using a nested evaluation protocol with held-out test sets and inner cross-validation. For fault classification on the Petrobras 3W dataset (208{,}973 windows, 10 classes), a statistical feature extraction pipeline compresses 720-step windows into 14-descriptor feature matrices; DeepONet achieves the best held-out accuracy of 96.83\% (macro $F_1{=}0.964$), with three of four trained models reaching $>$96.5\% after hyperparameter optimization, while the MLP baseline collapses to single-class prediction (6.5\% accuracy), revealing that temporal structure in the feature matrix is essential. TiRex provides a competitive zero-shot baseline (91.2\%). For gas production forecasting on the Ganymede dataset (31{,}359 samples, 7 wells), foundation models dominate: TimesFM achieves $R^2_\text{prod}{=}0.83$ at the 7-day horizon, exceeding the best trained model (PatchTST, 0.59) by 40\%; XGBoost produces negative $R^2_\text{prod}$ at all horizons despite Optuna HPO, confirming that flattened temporal features lose the sequential structure forecasting requires. Statistical significance is confirmed via Friedman tests on both tasks ($p < 0.05$).
\end{abstract}
...
\begin{enumerate}
    \item A \textbf{systematic benchmark} of eight architectures---five trained (LSTM, DeepONet, MLP, PatchTST, XGBoost) and three foundation models (Chronos, TimesFM, TiRex)---across two real-world offshore datasets covering fault classification and production forecasting.
...
As a non-sequential baseline, we include a feedforward MLP that flattens the $(14 \times 27)$ feature matrix into a 378-dimensional vector and applies two FC$\to$BatchNorm$\to$GELU$\to$Dropout blocks with hidden dimensions $[256, 128]$. Unlike the recurrent, operator-learning, and attention-based architectures above, this model treats the feature matrix as an unstructured vector, discarding all positional relationships among descriptors and sensors. Total parameters: 132K (classification only).

\subsubsection{XGBoost Baseline}

As a gradient-boosted tree baseline for Ganymede forecasting, we wrap \texttt{XGBRegressor} in scikit-learn's \texttt{MultiOutputRegressor} to produce multi-step forecasts. The 90-timestep $\times$ 63-feature input window is flattened into a 5{,}670-dimensional vector---like the MLP, this discards all temporal structure, treating the input as an unordered bag of features. We use histogram-based splitting (\texttt{tree\_method=hist}), 500 estimators, max depth~6, and L1/L2 regularization. Unlike the neural models, XGBoost requires no GPU and trains via coordinate descent on the gradient-boosted objective. This baseline tests whether ensemble tree methods can compensate for the loss of temporal structure through their superior handling of tabular features.
...
\toprule
\textbf{Parameter} & \textbf{LSTM} & \textbf{DeepONet} & \textbf{MLP} & \textbf{PatchTST} \\
\midrule
Learning rate      & 1e-3 & 5e-4 & 1e-3 & 1e-4 \\
Weight decay       & 1e-4 & 1e-4 & 1e-4 & 1e-2 \\
Batch size         & 64   & 64   & 64   & 64   \\
LR scheduler       & OneCycle & Cosine & Cosine & OneCycle \\
...
Table~\ref{tab:3w_main} presents the primary results on the held-out test set ($n{=}41{,}515$). All five models are evaluated on identical test samples. DeepONet and LSTM achieve near-identical accuracy ($\sim$96.8\%), while PatchTST reaches 96.5\% after Optuna HPO (30 trials, improved from 95.7\% with manual tuning). The MLP baseline collapses to single-class prediction (6.5\% accuracy), demonstrating that a flat feedforward architecture cannot exploit the feature matrix without sequential or structural inductive biases. Inner CV estimates are within 2\% of test performance for the successful models, indicating stable generalization.
...
MLP      & .065 & .012 & .008 & .100 & .174$\pm$.000 \\
\midrule
...
Fig.~\ref{fig:recall} shows per-class recall on the held-out test set for all four models. Classes~5 (flow instability), 6 (rapid productivity loss), and 9 (hydrate formation) achieve $>$99\% recall across trained models. The most challenging classes are 2 (incipient BSW increase, 89--90\% recall) and 4 (severe slugging, 95\%), which are also the rarest and most gradual-onset fault types. PatchTST shows reduced recall on class~8 (slow restriction, 87\%), while TiRex shows the largest drops on class~2 (67.8\%) and class~4 (73.5\%), indicating that frozen embeddings lose fine-grained discriminative information for rare event types.
...
\legend{LSTM, DeepONet, PatchTST, TiRex}
\end{axis}
\end{tikzpicture}
\caption{Per-class recall on the 3W held-out test set. Classes: 0=Normal, 1=Abrupt BSW$\uparrow$, 2=Incipient BSW$\uparrow$, 3=Spurious closure, 4=Severe slugging, 5=Flow instability, 6=Productivity loss, 7=Quick restriction, 8=Slow restriction, 9=Hydrate.}
\label{fig:recall}
\end{figure}
...
\addplot[color=xgboost, mark=square*, mark size=2.5pt, line width=1.2pt, forget plot] coordinates {(7,-0.031)(14,-0.465)(30,-0.415)};
\addplot[dashed, gray, domain=0:95] {0};
\legend{TimesFM, TiRex, Chronos, PatchTST, LSTM}
\end{axis}
\end{tikzpicture}
\caption{$R^2_\text{prod}$ vs.\ forecast horizon (Ganymede, held-out test, multi-well). Foundation models (top 3 curves) consistently outperform trained models. DeepONet omitted ($R^2 < -5$). XGBoost (\protect\tikz[baseline=-0.5ex]{\protect\draw[xgboost, fill=xgboost] (0,0) rectangle (0.25em,0.5em);}) shown for $h{\leq}30$; its $h{=}90$ value ($R^2{=}-1.25$) falls below the axis range. Dashed line: $R^2{=}0$ (naive baseline).}
\label{fig:ganymede}
\end{figure}
...
Three of the four trained classification models achieve strong performance ($>$96.5\% accuracy after HPO), suggesting that the statistical feature extraction captures the essential fault signatures when paired with architectures that model sequential or structural dependencies. DeepONet's flat MLP branch is the most parameter-efficient (209K) while marginally leading in accuracy (96.83\%). Optuna hyperparameter optimization (30 trials per model) improved PatchTST from 95.7\% to 96.5\% by finding a deeper, narrower architecture (4 layers, $d_\text{model}{=}128$), while LSTM and DeepONet showed no improvement---confirming that manual tuning was near-optimal for these architectures.

The MLP baseline's complete failure (6.5\% accuracy, single-class prediction) is instructive: despite receiving the same $(14 \times 27)$ feature matrix as the other trained models, flattening it into a 378-dimensional vector destroys the descriptor$\times$sensor structure that the other architectures exploit. This confirms that the statistical features contain discriminative information \emph{in their spatial arrangement}, not merely in their aggregate statistics---even the simplest structured model (DeepONet at 209K parameters) succeeds where a comparably-sized unstructured model (MLP at 132K) fails entirely.
...
XGBoost's negative $R^2_\text{prod}$ at all horizons (best: $-0.031$ at $h{=}7$, worst: $-1.25$ at $h{=}90$) mirrors the MLP's complete failure on 3W classification, providing convergent evidence that \emph{flattened features lose the structural information required for both tasks}. On 3W, the MLP flattened the $(14 \times 27)$ descriptor$\times$sensor matrix into a 378-dim vector and collapsed to single-class prediction; on Ganymede, XGBoost flattens the $(90 \times 63)$ temporal window into a 5{,}670-dim vector and performs worse than a naive mean predictor. Despite XGBoost's well-known strength on tabular data, the 5{,}670 features created by flattening lack the temporal ordering that LSTM, PatchTST, and the foundation models exploit. This consistent failure across both tasks and both flattening baselines strengthens the conclusion that inductive biases preserving temporal or spatial structure are essential for offshore monitoring.
...
We apply the Friedman test with Nemenyi post-hoc analysis to the inner CV fold-level metrics (5 folds for 3W, 3 folds for Ganymede). For 3W classification with five models, the Friedman test is significant across all three metrics ($p < 0.002$). Average ranks place PatchTST first (1.4), DeepONet second (2.0--2.2), LSTM third (2.4--2.6), TiRex fourth (4.0), and MLP last (5.0) for accuracy and macro $F_1$---the MLP's consistent last-place rank across all folds confirms its failure is not fold-dependent.

Interestingly, on AUC-PR the ranking shifts: TiRex ranks first (1.0) with PatchTST second (2.4), while MLP remains last (5.0), reflecting TiRex's well-calibrated probability estimates from the Random Forest on the high-dimensional embedding space.

For Ganymede forecasting, MAE reaches significance at $h{=}7$ ($p{=}0.014$), $h{=}14$ ($p{=}0.035$), and $h{=}30$ ($p{=}0.019$), with limited statistical power from 3 inner CV folds.

\subsection{Limitations}

This study has several limitations: (i) hyperparameter optimization was performed for the 3W classification task and for XGBoost on Ganymede forecasting, but not for the remaining trained models on Ganymede, where the large gap between FMs and trained models may partly reflect suboptimal tuning; and (ii) no interpretability analysis (e.g., attention visualization, SHAP values) has been conducted.
...
We benchmarked eight deep learning architectures on offshore fault classification and gas production forecasting using a rigorous nested evaluation protocol. Key findings:

\begin{enumerate}
    \item \textbf{Statistical features enable efficient classification}: Compressing 720-step windows into 14-descriptor matrices yields $>$96.5\% accuracy on 3W fault detection with three of four trained models (after HPO), while reducing training time by $\sim$50$\times$.
    \item \textbf{Feature matrix structure matters}: The MLP baseline (6.5\% accuracy) demonstrates that the discriminative information in the statistical feature matrix resides in the descriptor$\times$sensor arrangement, not the raw values---even the simplest structured model succeeds where a flat feedforward network fails entirely.
    \item \textbf{Flattened features fail across tasks and model families}: XGBoost, despite its strength on tabular data, produces negative $R^2_\text{prod}$ at all Ganymede horizons when given the flattened 5{,}670-dim input---converging with MLP's 3W failure to confirm that temporal/spatial structure preservation is essential for offshore monitoring.
    ...
    \item \textbf{Foundation models dominate forecasting}: TimesFM, TiRex, and Chronos all achieve $R^2_\text{prod} > 0.7$ at $h{=}7$ without any task-specific training, exceeding the best trained model by 24--40\%. TiRex maintains the strongest performance at long horizons ($R^2_\text{prod}{=}0.57$ at $h{=}90$).
...
Future work includes Ganymede hyperparameter optimization for the remaining trained models, model interpretability analysis, and evaluation of domain-adapted foundation models.
