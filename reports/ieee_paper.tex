...
\begin{enumerate}
    \item A \textbf{systematic benchmark} of ten architectures---seven trained (LSTM, DeepONet, Random Forest, PatchTST, TCN, FKM-AD, MambaSL) and three foundation models (Chronos, TimesFM, TiRex)---across two real-world offshore datasets covering fault classification and production forecasting.
...
We evaluate MambaSL on the same dual data paths as FKM-AD: the \textit{feature} path operates on pre-extracted $(14 \times 27)$ statistical matrices, and the \textit{raw} path operates directly on $(720 \times 27)$ sensor windows with per-instance z-score normalization. MambaSL results are at baseline hyperparameters (pre-HPO).
...
Table~\ref{tab:3w_main} presents the primary results on the held-out test set ($n{=}41{,}515$). All nine model configurations are evaluated on identical test samples. DeepONet and LSTM achieve near-identical accuracy ($\sim$96.8\%), while PatchTST reaches 96.5\% after Optuna HPO (30 trials, improved from 95.7\% with manual tuning). Notably, Random Forest achieves 96.58\% accuracy (0.966, macro $F_1{=}0.964$, AUC-PR${=}0.985$) on the flattened 378-dimensional feature vector, demonstrating that the statistical feature extraction pipeline captures discriminative information accessible even to non-sequential, tabular learners. FKM-AD on features achieves 96.7\% accuracy ($F_1$m${=}0.961$), competitive with the top models, while the raw-window variant reaches 94.9\%---a 1.8 percentage point gap that quantifies the value of the feature extraction pipeline for state-space models. Inner CV estimates are within 2\% of test performance for all trained models, indicating stable generalization.

MambaSL \cite{mambasl} provides a second dual-path evaluation on the same classification task. The feature path achieves 96.1\% accuracy ($F_1$m${=}0.947$, AUC-PR${=}0.904$, CV${=}95.3\%$), placing it between FKM-AD (feat.) and TiRex in the accuracy ranking. The raw-window variant reaches 90.1\% ($F_1$m${=}0.867$, CV${=}89.4\%$)---a 6.0~pp feature-vs-raw gap that is substantially wider than FKM-AD's 1.8~pp gap. This larger gap suggests that MambaSL's single-layer time-variant SSM design, which lacks FKM-AD's explicit Fourier-basis frequency decomposition, is more dependent on pre-extracted statistical features when processing the high-dimensional 27-sensor input space. Both MambaSL results are at baseline hyperparameters; HPO is expected to improve these numbers.
...
MambaSL (feat.)$^\ddagger$ & .961 & .947 & .962 & .904 & .953$\pm$.013 \\
MambaSL (raw)$^\ddagger$   & .901 & .867 & .903 & .779 & .894$\pm$.021 \\
\midrule
...
\multicolumn{6}{@{}l}{\scriptsize $^\ddagger$MambaSL at baseline hyperparameters (HPO pending).}
\end{tabular}
\end{table}
...
\addplot[fill=mambasl, draw=mambasl!80!black] coordinates {
    (0,0.903)(1,0.934)(2,0.905)(3,0.978)(4,0.926)
    (5,0.994)(6,0.999)(7,0.960)(8,0.969)(9,0.999)
};
\legend{LSTM, DeepONet, PatchTST, RF, FKM-AD, TiRex, MambaSL}
\end{axis}
\end{tikzpicture}
\caption{Per-class recall on the 3W held-out test set for all seven trained models plus TiRex. Classes: 0=Normal, 1=Abrupt BSW$\uparrow$, 2=Incipient BSW$\uparrow$, 3=Spurious closure, 4=Severe slugging, 5=Flow instability, 6=Productivity loss, 7=Quick restriction, 8=Slow restriction, 9=Hydrate.}
\label{fig:recall}
\end{figure}
...
MambaSL \cite{mambasl} provides additional evidence that widens the picture. With a single-layer time-variant SSM and multi-head adaptive attention pooling (0.43M parameters), MambaSL achieves 96.1\% accuracy on features (CV${=}95.3\%$) but only 90.1\% on raw windows (CV${=}89.4\%$)---a 6.0~pp feature-vs-raw gap. This is more than three times FKM-AD's 1.8~pp gap, suggesting that MambaSL's architectural choices---in particular, the absence of Fourier-basis frequency pre-conditioning---make it substantially more sensitive to the quality of input representation. The pattern is consistent across the class recall analysis: MambaSL (feat.) achieves $>$90\% recall on every class, while the raw variant struggles with class~2 (incipient BSW) and class~4 (severe slugging). Taken together, FKM-AD and MambaSL establish that the feature extraction benefit is a structural property of state-space models applied to this domain: the magnitude of the benefit varies with the SSM's frequency-handling capability, but it is positive in both cases.
...
We apply the Friedman test with Nemenyi post-hoc analysis to the inner CV fold-level metrics (5 folds for 3W, 3 folds for Ganymede). For 3W classification with nine model configurations (including both FKM-AD and MambaSL paths), the Friedman test is significant across all three metrics: accuracy ($\chi^2{=}35.5$, $p < 0.001$), macro $F_1$ ($\chi^2{=}34.5$, $p < 0.001$), and AUC-PR ($\chi^2{=}34.3$, $p < 0.001$). Average accuracy ranks place Random Forest first (1.0), FKM-AD~features second (2.8), PatchTST third (3.2), MambaSL~features fourth (4.8), DeepONet fifth (4.2), LSTM sixth (5.0), FKM-AD~raw seventh (7.0), TiRex eighth (8.4), and MambaSL~raw ninth (8.6). With Nemenyi CD${}=5.37$ at $\alpha{=}0.05$, significant pairwise differences are found between: FKM-AD~features and MambaSL~raw (rank diff$=5.8$); FKM-AD~features and TiRex (rank diff$=5.6$); FKM-AD~raw and Random Forest (rank diff$=6.0$); MambaSL~raw and PatchTST (rank diff$=5.4$); MambaSL~raw and Random Forest (rank diff$=7.6$); and Random Forest and TiRex (rank diff$=7.4$). These results confirm that FKM-AD and MambaSL on features are statistically indistinguishable from the top models, while both raw-window variants perform significantly worse than Random Forest or are significantly outperformed by feature-path SSMs.
...
We benchmarked nine architectures on offshore fault classification and gas production forecasting using a rigorous nested evaluation protocol. Key findings:
...
\begin{enumerate}
    ...
    \item \textbf{MambaSL widens the feature-extraction case for SSMs}: MambaSL \cite{mambasl} (single-layer time-variant SSM) achieves 96.1\% on pre-extracted features but only 90.1\% on raw windows at baseline hyperparameters---a 6.0~pp gap three times larger than FKM-AD's. The magnitude of the feature-extraction benefit varies with the SSM's frequency-handling capability, but it is positive for both architectures, strengthening the argument that statistical feature extraction is the primary performance driver for SSMs on this task.
    \item \textbf{DeepONet leads classification, fails forecasting}: The operator-learning framework achieves the best classification accuracy (96.85\%) with the fewest parameters (209K) but produces negative $R^2$ on all forecasting horizons.
...
Future work includes hyperparameter optimization for MambaSL and the remaining trained models on forecasting, model interpretability analysis, and evaluation of domain-adapted foundation models.
