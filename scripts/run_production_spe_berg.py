...
def _run_trained_model(
    ...
    if well:
        ...
        out_path = RESULTS_DIR / model_name / f"spe_berg_h{horizon}_{mode}_{safe_name}.json"
    else:
...
def _run_fm_per_well(
    ...
    for well in WELLS:
        ...
        out_path = RESULTS_DIR / model_name / f"spe_berg_h{horizon}_per_well_{safe}.json"
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(_make_serializable(result), indent=2))
        logger.info("  Saved %s", out_path)
        ...
        try:
            if is_fm:
                if mode == "multi_well":
                    ...
                    out_path = RESULTS_DIR / model / f"spe_berg_h{horizon}_per_well_{safe}.json"
                    out_path.parent.mkdir(parents=True, exist_ok=True)
                    out_path.write_text(json.dumps(_make_serializable(result), indent=2))
                    logger.info("  Saved %s", out_path)
            elif is_tree:
