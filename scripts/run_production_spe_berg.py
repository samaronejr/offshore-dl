"""Production sweep: 7 models × 4 horizons × 2 modes on SPE BERG.

Orchestrates all production runs for the SPE BERG Eagle Ford shale gas
...
Usage::
    ...
    python scripts/run_production_spe_berg.py

    # Smoke test (CPU, 1 epoch, single model)
    python scripts/run_production_spe_berg.py --max-epochs 1 --device cpu --models lstm

    # Dry run — print plan without executing
    python scripts/run_production_spe_berg.py --dry-run

    # Docker invocation
    docker_run.sh python scripts/run_production_spe_berg.py --device cuda
"""
...
# SPE BERG: 53 wells (well_1 … well_53)
WELLS = [f"well_{i}" for i in range(1, 54)]
...
def _run_trained_model(
    ...
    if well:
        ...
        out_path = (
            RESULTS_DIR / model_name / f"spe_berg_h{horizon}_{mode}_{safe_name}.json"
        )
    else:
        out_path = RESULTS_DIR / model_name / f"spe_berg_h{horizon}_{mode}.json"
    ...
    print(
        f"\n  {model_name.upper()} on SPE BERG h{horizon} {mode}"
        f"{(' ' + well) if well else ''}"
    )
...
def _run_fm_multi_well(
    ...
    """Run zero-shot FM on SPE BERG multi_well with temporal holdout.
    ...
    D028: target_channel must be passed because SPE BERG's target
    (Gas_Volume_MMscf) does NOT sort to index 0 in the common columns.
    """
    set_global_seed(42)
    dataset = SPEBergDataset(
        "configs/data/spe_berg.yaml",
        horizon=horizon,
        mode="multi_well",
    ...
    # Save
    out_path = RESULTS_DIR / model_name / f"spe_berg_h{horizon}_multi_well.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def _run_fm_per_well(
    ...
    """Run zero-shot FM on each SPE BERG well for one horizon.

    Batches by model: creates FM once per well (n_vars can vary), iterates wells.

    D028: target_channel must be passed — Gas_Volume_MMscf is not at index 0.
    """
    fm_class = _load_fm_class(model_name)
    per_well_results = []
    ...
    for well in WELLS:
        ...
        out_path = (
            RESULTS_DIR / model_name / f"spe_berg_h{horizon}_per_well_{safe}.json"
        )
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(json.dumps(_make_serializable(result), indent=2))
        logger.info("  Saved %s", out_path)
...
def _print_plan(plan: list[dict]) -> None:
    ...
    print(f"\n{'═' * 70}")
    print(f"  SPE BERG PRODUCTION SWEEP PLAN — {len(plan)} runs")
    print(f"{'═' * 70}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Production sweep: SPE BERG Eagle Ford shale gas forecasting",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g. well_12 well_13) or a Python-style slice START:END "
    ...
    logger.info("═" * 70)
    logger.info("SPE BERG PRODUCTION SWEEP — %d runs", len(plan))
    logger.info(
    ...
    for run_spec in plan:
        ...
        if well:
            ...
            out_path = (
                RESULTS_DIR / model / f"spe_berg_h{horizon}_per_well_{_skip_safe}.json"
            )
            ...
            if is_fm:
                if mode == "multi_well":
                    ...
                    out_path = (
                        RESULTS_DIR
                        / model
                        / f"spe_berg_h{horizon}_per_well_{safe}.json"
                    )
                    out_path.parent.mkdir(parents=True, exist_ok=True)
                    out_path.write_text(
    ...
    for model, statuses in all_status.items():
        summary_path = RESULTS_DIR / model / "summary_production_spe_berg.json"
    ...
    print(f"\n{'═' * 70}")
    print(f"  SPE BERG PRODUCTION SWEEP COMPLETE")
    print(f"{'═' * 70}")
