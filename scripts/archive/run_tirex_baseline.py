...
def run_ganymede() -> dict:
    """TiRex zero-shot on Ganymede forecasting."""
    set_global_seed(42)
...
def run_cdf() -> dict:
    """TiRex zero-shot on CDF anomaly detection."""
    set_global_seed(42)
...
def main() -> None:
    results_dir = Path("results/tirex")
    ...
    for name, fn in runs:
        logger.info("=" * 60)
        logger.info("Starting TiRex on %s", name)
        logger.info("=" * 60)
