"""Run PatchTST baseline on all 3 datasets and save results.

CPU-feasible verification with small architecture and epoch counts.

Usage:
    python scripts/run_patchtst_baseline.py
"""
...
# Small architecture for CPU baseline
SMALL_ARCH = {"patch_len": 16, "stride": 8, "d_model": 32, "n_heads": 4, "n_layers": 1, "d_ff": 64, "dropout": 0.1}
...
def run_cdf() -> dict:
    ...
    runner = ExperimentRunner(
        model_class=PatchTSTModel, dataset=dataset, cv_strategy=cv, cfg=cfg,
        model_kwargs={"task": "anomaly", "n_vars": 11, "window_size": 48, **SMALL_ARCH},
    )
...
def run_ganymede() -> dict:
    ...
    runner = ExperimentRunner(
        model_class=PatchTSTModel, dataset=dataset, cv_strategy=cv, cfg=cfg,
        model_kwargs={"task": "forecasting", "n_vars": n_vars, "horizon": 30, "window_size": 90, **SMALL_ARCH},
    )
    return runner.run(use_mlflow=False)
...
def run_3w() -> dict:
    ...
    runner = ExperimentRunner(
        model_class=PatchTSTModel, dataset=dataset, cv_strategy=cv, cfg=cfg,
        model_kwargs={"task": "classification", "n_vars": 27, "n_classes": 10, "window_size": 720, **SMALL_ARCH},
    )
    return runner.run(use_mlflow=False)
...
def main() -> None:
    results_dir = Path("results/patchtst")
    ...
    for name, fn in runs:
        logger.info("=" * 60)
        logger.info("Starting PatchTST on %s", name)
        logger.info("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  PATCHTST BASELINE RESULTS")
    print("=" * 60)
