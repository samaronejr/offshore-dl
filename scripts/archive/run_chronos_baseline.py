"""Run Chronos zero-shot baseline on Ganymede and CDF.

No training — just inference. Classification (3W) not supported.
Uses small subsets for CPU-feasible verification.

Usage:
    python scripts/run_chronos_baseline.py
"""
...
def _zero_shot_evaluate(model, dataset, val_idx, task, batch_size=32, max_samples=200):
    ...
    # Limit samples for CPU feasibility
...
def run_ganymede() -> dict:
    """Chronos zero-shot on Ganymede forecasting."""
    set_global_seed(42)
...
def run_cdf() -> dict:
    """Chronos zero-shot on CDF anomaly detection."""
    set_global_seed(42)
...
def main() -> None:
    ...
    for name, fn in runs:
        logger.info("=" * 60)
        logger.info("Starting Chronos on %s", name)
        logger.info("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  CHRONOS ZERO-SHOT RESULTS")
    print("=" * 60)
