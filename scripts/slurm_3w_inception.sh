...
#SBATCH --output=logs/3w_inception_%j.out
#SBATCH --error=logs/3w_inception_%j.err

echo "=== InceptionTime 3W === Node: $(hostname) Date: $(date)"
nvidia-smi -L
PROJECT=~/offshore-dl
...
singularity exec --nv --no-home \
    ...
    --pwd /app $SIF \
    python scripts/run_production_3w_features.py --device cuda --models inception_time

echo "=== InceptionTime DONE: $(date) ==="
