#!/bin/bash
# Run all 16 SPE BERG HPO studies sequentially
# Usage: bash scripts/run_spe_berg_hpo_all.sh [logfile]
# Output: results/hpo/spe_berg/{model}_{horizon}.json for all 16 combos

set -euo pipefail

LOGFILE="${1:-logs/spe_berg_hpo_all.log}"
mkdir -p "$(dirname "$LOGFILE")"
mkdir -p results/hpo/spe_berg
...
for model in "${MODELS[@]}"; do
  for h in "${HORIZONS[@]}"; do
    total=$((total + 1))
    out="results/hpo/spe_berg/${model}_${h}.json"
...
# Final count
count=$(ls results/hpo/spe_berg/*.json 2>/dev/null | wc -l)
